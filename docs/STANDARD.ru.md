...
Этот документ описывает нормативное ядро стандарта. Слова `MUST`, `SHOULD` и `MAY` используются в техническом смысле:
...
Стандарт задает единый API-профиль для регулируемых площадок цифровой валюты и цифровых активов:
...
- market makers;
- custodians;
- регуляторные и аналитические системы;
- разработчики торгового и учетного ПО.

## 2. Базовая архитектура

Площадка, совместимая со стандартом, предоставляет:
...
- `instrument_id`: глобальный id инструмента внутри площадки;
...
- fee estimate;
- travel rule metadata, если требуется правом.
