...
Этот документ описывает нормативное техническое ядро RU Digital Market Interoperability Profile. Существующие торговые контракты L1-L4 сохраняются для обратной совместимости внутри более широкого профиля рынка. Документ не является нормативным правовым актом. Слова `MUST`, `SHOULD` и `MAY` используются только для проверки заявленного уровня API-совместимости:
...
Реализация, заявляющая соответствующий уровень совместимости, предоставляет:
...
Клиентская и серверная бизнес-логика SHOULD принимать решения по capabilities, состояниям, risk policy и execution contract, а не по имени участника или внутреннему типу адаптера.
...
Повтор команды с тем же idempotency key и тем же содержанием MUST возвращать исходный результат. Тот же ключ с другим содержанием MUST завершаться ошибкой конфликта без выполнения команды.
...
Реализации MAY добавлять vendor-specific fields через объект `extensions`, не нарушая базовую схему.
...
Если capability зависит от регистрации, лицензии, реестра, правил участника или ЭПР, `/v1/profile` MUST показывать это через `legal_profiles.status`, а не через маркетинговое описание. Статус `supported` означает техническую реализацию и сам по себе не означает правовой допуск.

Значения `legal_profiles.framework_id` являются идентификаторами RU-DMIP для discovery. Они не заменяют реквизиты нормативного акта; официальные источники и актуальная дата проверки указываются отдельно.
...
Для HMAC-подписи private REST запросов базовая строка подписи: `timestamp + method + path + canonical_query + sha256(body)`. `canonical_query` сохраняет все пары параметров, включая повторяющиеся: имена и значения кодируются в UTF-8 и percent-encoding по RFC 3986, пробел передается как `%20`, затем закодированные пары сортируются побайтово по имени и значению. Хеш тела вычисляется по точным передаваемым байтам. Production-реализация MUST проверять подпись, окно timestamp, nonce или replay cache, scope ключа и требуемый уровень аутентификации до оценки действия.
